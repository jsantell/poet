...
function init (poet, callback) {
  ...
  if (options.useDb){
      var promise = db.getPostsFromDb(options.posts).then(function (posts) {
          // Generate a collection of promises that resolve
          // to post objects
          var collection = posts.reduce(function (coll, postObj) {
              var template = utils.getTemplate(poet.templates, postObj.template);
              ...
              var post = utils.createPostFromDb(postObj, options).then(function (post) {
...
function watch (poet, callback) {
  if (poet.options.useDb){
      db.registerPostSaveMiddleware(poet, callback)
  } else {
