...
function createOptions (options) {
  ...
  options = options || {};
  Object.keys(defaults).reduce(function (coll, prop) {
    if (prop === 'routes') {
      // Use any routes specified by user
      coll.routes = {};
      defaults.routes.map(function (route) {
        var routes = options.routes || {};
        var type = route.match(/\(:\w*)$/)[1];
        var match = routes.reduce(function (res, userRoute) {
          if (~userRoute.indexOf(type)) return userRoute;
        });
  ...
  createDefaults = require('./poet/defaults'),
}
/**
