 ...
 * (http://expressjs.com/3x/api.html#app.use) the path created has to observe the
 * mounting point, otherwise created links would be broken.
 *
 * @params {String} route
 * @params {String} value
 * @returns {String}
...
function createURL (poet, route, value) {
  ...
  return poet.app ? path.join(poet.app.route, baseURL) : baseURL;
}
...
function createPost (poet, filePath, options) {
  ...
  return fs.readFile(filePath, 'utf-8').then(function (data) {
    ...
    if (!isAbsoluteRoute) post.url = post.url.replace(/^\//, '');
