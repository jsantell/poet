...
function getTags (poet) {
  return poet.cache.tags || (poet.cache.tags = (poet.options.useDb ? db.getTags() : utils.getTags(getPosts(poet))));
}
...
function getCategories (poet) {
  return poet.cache.categories ||
    (poet.cache.categories = (poet.options.useDb ? db.getCategories() : utils.getCategories(getPosts(poet))));
}
