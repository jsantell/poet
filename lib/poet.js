...
function init ( callback ) {
  ...
  function read_dir(dir) {
    fs.readdir(dir, function ( err, files ) {
      ...
      files.forEach(function ( file ) {
        var sync = fs.lstatSync(dir + file);
        if (sync.isDirectory()) {
          --totalFiles;
          read_dir(dir + file + '/');
          return;
        ...
        fs.readFile( dir + file, 'utf-8', function ( err, data ) {
          saveData( err, data, dir, file, template, function () {
            if ( !--totalFiles ) {
              refs();
              locals();
  ...
  read_dir(options.posts);
}

function saveData ( err, dir, data, file, template, callback ) {
  var
    t = options.metaFormat( data ),
    body = t.body,
    attributes = t.attributes,
    fileName = (dir + file).replace( /\.[^\.]*$/, '' ),
