...
var
  allowedTags = 'a|b|blockquote|code|del|dd|dl|dt|em|h1|h2|h3|'+
                'i|img|li|ol|p|pre|sup|sub|strong|strike|ul|br|hr|script',
  allowedAttrs = {
...
function saveData ( err, data, file, callback ) {
  ...
  storage.posts[ fileName ].content = md( t.body, true, allowedTags, allowedAttrs );
  storage.posts[ fileName ].url = options.routes.post.match( /[^\:]*/ )[0] + fileName;

  storage.posts[ fileName ].preview = md(
    storage.posts[ fileName ].preview || t.body.replace(/\n.*/g, ''), true, allowedTags, allowedAttrs
  );
  callback();
}
