...
module.exports = function poet ( _app, userOptions ) {
  storage = {
    posts            : {},
    tags             : [],
    postsTagged      : {},
...
function saveData ( err, data, file, template, callback ) {
  ...
  lengthDefinedPreview = post.previewLength 
      && !post.preview
      ? t.body.substr( 0, post.previewLength ) : undefined;
  readMoreDefinedPreview = t.body.indexOf("<!--more-->") >= 0 
      && !post.preview 
      && !post.previewLength 
      ? t.body.split('<!--more-->')[0] + options.readMore(post) : undefined;  
  post.preview = template( post.preview || lengthDefinedPreview || readMoreDefinedPreview || t.body.replace(/\n.*/g, ''));
  // If the user didn't use '<!--more-->' to place the readMore link, we should add it.
  if ( !readMoreDefinedPreview ) {
    post.preview += options.readMore ? options.readMore( post ) : '';
  }
  callback();
}
